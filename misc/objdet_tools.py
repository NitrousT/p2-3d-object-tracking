...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    for row in detections:
        ...
        this_color = color if color else configs.obj_colors[int(_id)]
        ...
        cv2.polylines(bev_map, [corners_int], isClosed=True, color=this_color, thickness=2)

        # draw front-facing line (between front-left and front-right)
        front_left  = (int(bev_corners[0, 0]), int(bev_corners[0, 1]))
        front_right = (int(bev_corners[3, 0]), int(bev_corners[3, 1]))
        cv2.line(bev_map, front_left, front_right, (255, 255, 0), 2)
